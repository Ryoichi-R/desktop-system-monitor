...
public partial class MainWindow : Window
{
    ...
    public void ApplyVisualSettings(AppSettings settings, bool batteryPresent = false)
    {
        ...
        const double informationWidth = 280d;
        ...
        CpuRow.Visibility = settings.ShowCpuMetrics ? Visibility.Visible : Visibility.Collapsed;
        GpuRow.Visibility = settings.ShowGpuMetrics ? Visibility.Visible : Visibility.Collapsed;
        DiskRow.Visibility = settings.ShowDiskMetrics ? Visibility.Visible : Visibility.Collapsed;
        BatteryRow.Visibility = settings.ShowBatteryEstimate && batteryPresent ? Visibility.Visible : Visibility.Collapsed;
        CpuTemperatureColumn.Width = new GridLength(settings.ShowCpuMetrics && settings.EffectiveShowCpuTemperature ? 44d : 0d);
        GpuTemperatureColumn.Width = new GridLength(settings.ShowGpuMetrics && settings.EffectiveShowGpuTemperature ? 44d : 0d);
        FrameworkElement[] orderedRows = [CpuRow, MemoryRow, GpuRow, DiskRow, NetworkRow, BatteryRow];
        FrameworkElement[] visibleRows = orderedRows.Where(row => row.Visibility == Visibility.Visible).ToArray();
        foreach (FrameworkElement row in orderedRows)
        {
            row.Margin = new Thickness(0, Array.IndexOf(visibleRows, row) > 0 ? WidgetHeightCalculator.RowGap : 0, 0, 0);
        }
        double informationHeight = WidgetHeightCalculator.Calculate(visibleRows.Length);
        WidgetRoot.Height = informationHeight;
        BackgroundPresentationMode backgroundMode = BackgroundPresentationPolicy.Evaluate(settings);
        double fadeExpansionRatio = backgroundMode == BackgroundPresentationMode.None
            ? 0
            : BackgroundBrushFactory.EdgeFadeExpansionRatio(settings);
        Width = informationWidth * scale * (1d + fadeExpansionRatio);
        Height = informationHeight * scale * (1d + fadeExpansionRatio);
        Visibility metricPeakVisibility = settings.ShowRecentPeaks ? Visibility.Visible : Visibility.Collapsed;
        ...
        NetworkPeakRow.Visibility = settings.EffectiveShowNetworkPeaks ? Visibility.Visible : Visibility.Collapsed;
        FontFamily = new MediaFontFamily(settings.FontFamilyName);
        Resources["WidgetForeground"] = new SolidColorBrush((MediaColor)MediaColorConverter.ConvertFromString(settings.ForegroundColor));
        Resources["WidgetMuted"] = new SolidColorBrush((MediaColor)MediaColorConverter.ConvertFromString(settings.MutedColor));
        Resources["WidgetAccent"] = new SolidColorBrush((MediaColor)MediaColorConverter.ConvertFromString(settings.AccentColor));
        Resources["WidgetRxAccent"] = new SolidColorBrush((MediaColor)MediaColorConverter.ConvertFromString(settings.RxAccentColor));
        Resources["WidgetTxAccent"] = new SolidColorBrush((MediaColor)MediaColorConverter.ConvertFromString(settings.TxAccentColor));
