...
public partial class SettingsWindow : Window
{
    ...
    /// <summary>
    /// Brings the dialog forward. <paramref name="useTopmostPulse"/> should be
    /// true only when the Owner (MainWindow) is BottomMost: a one-shot
    /// Topmost pulse is needed there because the owner itself never sits above
    /// other windows. For a TopMost or Normal owner, Owner-relative Z-order
    /// already keeps this dialog above or level with the owner, so pulsing
    /// Topmost here would only cause an extra, unnecessary toggle.
    /// </summary>
    internal void RequestForegroundPresentation(bool useTopmostPulse)
    {
        if (useTopmostPulse && !Topmost)
        {
            Topmost = true;
            _restoreTopmostAfterForegroundPresentation = true;
        }

        if (!IsVisible)
        {
            if (!_foregroundPresentationWaitingForContent)
            {
                _foregroundPresentationWaitingForContent = true;
                ContentRendered += OnForegroundPresentationContentRendered;
            }
            return;
        }

        _ = Activate();
        QueueForegroundTopmostRestore();
    }

    private void OnForegroundPresentationContentRendered(object? sender, EventArgs e)
    {
        ContentRendered -= OnForegroundPresentationContentRendered;
        _foregroundPresentationWaitingForContent = false;
        _ = Activate();
        QueueForegroundTopmostRestore();
    }

    private void QueueForegroundTopmostRestore()
    {
        if (!_restoreTopmostAfterForegroundPresentation)
        {
            return;
        }

        _restoreTopmostAfterForegroundPresentation = false;
    ...
    private void Save_Click(object sender, RoutedEventArgs e)
    {
        ...
        Result = new SettingsDraft
        {
            ...
            DisplayMode = Enum.Parse<WidgetDisplayMode>(SelectedTag(DisplayModeBox)),
