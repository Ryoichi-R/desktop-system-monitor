...
function Resolve-DesktopSystemMonitorCandidateIdentity {
    ...
    if (-not (Test-Path -LiteralPath $publishContractScript -PathType Leaf)) {
        throw "Publish contract helper is missing: $publishContractScript"
    }
    . $publishContractScript
    $publishDefinition = Get-DesktopSystemMonitorPublishContract
    $publishContract = [string]$publishDefinition.CandidateString
    ...
    [pscustomobject]@{
        ...
        dotnetSdk = $sdkVersion
        publishContract = $publishContract
        publishContractObject = $publishDefinition.Manifest
