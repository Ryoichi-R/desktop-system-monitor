...
public sealed class NotifyIconController : IDisposable
{
    ...
    private readonly Forms.ToolStripMenuItem _displayModeHeader;
    private readonly Forms.ToolStripMenuItem _standardDisplayModeItem;
    private readonly Forms.ToolStripMenuItem _reducedDisplayModeItem;
    ...
    public NotifyIconController()
    {
        ...
        _displayModeHeader = new Forms.ToolStripMenuItem("表示幅");
        _standardDisplayModeItem = new Forms.ToolStripMenuItem("通常幅（280 DIP）") { CheckOnClick = true };
        _reducedDisplayModeItem = new Forms.ToolStripMenuItem("縮小表示（150 DIP）") { CheckOnClick = true };
        _standardDisplayModeItem.Click += (_, _) => DisplayModeRequested?.Invoke(WidgetDisplayMode.Standard);
        _reducedDisplayModeItem.Click += (_, _) => DisplayModeRequested?.Invoke(WidgetDisplayMode.Reduced);
        _displayModeHeader.DropDownItems.Add(_standardDisplayModeItem);
        _displayModeHeader.DropDownItems.Add(_reducedDisplayModeItem);
        ...
        menu.Items.Add(_displayModeHeader);
    ...
    public void SetEditingEnabled(bool enabled)
    {
        ...
        _displayModeHeader.Enabled = enabled;
    ...
    public void SetDisplayMode(WidgetDisplayMode displayMode)
    {
        WidgetDisplayMode normalized = Enum.IsDefined(displayMode)
            ? displayMode
            : WidgetDisplayMode.Standard;
        _standardDisplayModeItem.Checked = normalized == WidgetDisplayMode.Standard;
        _reducedDisplayModeItem.Checked = normalized == WidgetDisplayMode.Reduced;
    }

    internal Forms.ToolStripMenuItem DisplayModeHeader => _displayModeHeader;
    internal Forms.ToolStripMenuItem StandardDisplayModeItem => _standardDisplayModeItem;
    internal Forms.ToolStripMenuItem ReducedDisplayModeItem => _reducedDisplayModeItem;
