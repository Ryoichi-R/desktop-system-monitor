...
public sealed class AppComposition : IDisposable
{
    ...
    private void ShowHighLoadProcesses()
    {
        ...
        _processWindow = new HighLoadProcessesWindow { Owner = _mainWindow };
    ...
    private void OnSessionSwitch(object sender, SessionSwitchEventArgs e)
    {
        Application.Current?.Dispatcher.BeginInvoke(() =>
        {
            LayerLifecycleCoordinator.HandleSessionSwitch(
                ...
                suspended =>
                {
                    _mainWindow?.SetLayerRepairSuspended(suspended);
                    _backgroundLayer?.SetRepairSuspended(suspended);
                },
                () =>
                {
                    _mainWindow?.ReapplyWindowStyles(LayerRepairTrigger.SessionUnlock);
