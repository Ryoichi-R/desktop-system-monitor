...
public sealed class DiskMetricSourceTests
{
    ...
    [Fact]
    public async Task does_not_rebuild_faster_than_the_retry_interval_when_collect_keeps_failing()
    {
        // Every rebuilt query still fails to Collect(), simulating a flaky
        // counter rather than a query handle that is simply broken.
        ...
        using var source = new DiskMetricSource(
            ...
            {
                queriesCreated++;
                return new FakeQuery { CollectResult = false };
            },
            () => now);
        source.SetEnabled(true, 0);
        Assert.Equal(1, queriesCreated);

        // First observed failure recovers immediately, same as
        // rebuilds_query_after_collect_failure.
        await source.SampleAsync(default);
        Assert.Equal(2, queriesCreated);

        // A second failure moments later must not trigger another rebuild.
        await source.SampleAsync(default);
        Assert.Equal(2, queriesCreated);

        // Once the retry interval has elapsed, a further failure may rebuild again.
        now = now.AddSeconds(5);
        await source.SampleAsync(default);

        Assert.Equal(3, queriesCreated);
    }
