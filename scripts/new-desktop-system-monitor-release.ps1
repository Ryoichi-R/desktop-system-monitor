...
function Test-ReleaseZip {
    ...
    Assert-PackageContents -Directory $ExtractionRoot
...
if (-not (Test-Path -LiteralPath (Join-Path $projectRoot 'LICENSE') -PathType Leaf)) {
...
try {
    foreach ($rid in $Runtime) {
        ...
        & (Join-Path $projectRoot 'scripts/publish-desktop-system-monitor.ps1') -Runtime $rid -OutputDir ([IO.Path]::GetRelativePath($projectRoot, $stage))
        ...
        Assert-PackageContents -Directory $stage
        ...
        Test-ReleaseZip -ArchivePath $archivePath -ExpectedRid $rid -ExpectedExecutableHash $packagedExecutableHash -ExtractionRoot $extractRoot
        $artifacts += [ordered]@{
            rid = $rid
